...
private:
    ...
    enum Message { EMPTY, RETRY, REVOKE };
    enum State { NONE, ACQUIRING, FREE, RELEASING, LOCKED };
    ...
    struct lock_entry {
        State state;
        Message message;
        std::queue<thread_cond *> waiting_threads;
    };

    pthread_mutex_t lockLock;
    std::map<lock_protocol::lockid_t, lock_entry *> locks;

    lock_protocol::status acquiring(lock_entry *, lock_protocol::lockid_t, bool);
